...
그 줄에서는 `false`값을 유지한다. 허나 `puts`로 `var`를 출력해보면 앞서 저장된 `true`가
출력된다. if문 처럼 한줄에 `true and false`를 쓰면 그 줄에서는 `false`가 유지되서 정상적인
...
not`보다 높다. 앞에서 ruby의 경우 논리 연산의 결과를 변수에 저장할 때는 괄호를 사용해야
