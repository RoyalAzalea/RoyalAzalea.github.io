...
위 결과를 보면 `c.m()`이 실행되고 `C1_class`의 method `m`이 실행됬다. 그이유가 바로
`class C3(C1, C2):` 이때 `C1_class`가 `C2_class`보다 먼저 쓰여있어서 우선순위 설정이
...
위 결과는 `C2_class`를 `C1_class`보다 먼저 적어 우선순위가 `C2_class`가 높다. 그러므로
