...
반대로 예측값($\hat{x}_k^{-}$)이 추정값에 미치는 영향은 더 커지게 된다.  
그런데 예측값은 이전 추정값에 비례하므로($\hat{x}_k^{-} = A\hat{x}_{k-1}$),  
새로운 추정값에는 이전 추정값($\hat{x}_{k-1}$)이 주로 반영된다.
**즉 칼만 이득이 작아지면, 추정값은 별로 변하지 않게 된다.** 추정 오차가 충분히 작아져서  더 이상 추정값이 변하지 않는 상태로 수렴한 것 이다.  
